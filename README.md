...
- **Refuses a network location** — a UNC path, a mapped network drive, or a path reached through a junction or symlink whose target is a share. Real values written there cross the wire and land on the server's storage (backups, snapshots, another machine's disk), where the unlink on exit cannot reach any copy it has already made. The UNC and junction checks run *before* the path is resolved, because resolving is itself the step that opens SMB with your credentials. What the junction check catches is **one hop**: a link whose own target is a share. A chain — a link to a local path that is itself a link to a share — is caught after resolution by the containment check, so the real values are still never written there, but `resolve()` will have touched the share first. Following the chain safely means validating every prefix of every target, which the one-hop walk does not do; a version that followed targets naively made this worse, not better, and was reverted (2.0.5 → 2.0.6).
...
Every program gets a value out of a `.env` in one of six ways. The full per-tool survey, with
sources, is [docs/env-consumption-research.md](docs/env-consumption-research.md); the executable
version is `tests/test_consumption_matrix.py`, which runs each mechanism through the vault with
the real `python-dotenv` and `pydantic-settings` (both ship with `mcp[cli]`), Node's built-in
`--env-file`, and bash.
...
`run_with_env(command=["code", "."], background=True, only_vars=[...])`.

Two things no mode can fix, because they are properties of the consumer: `docker run
--env-file`, `kubectl` and `make` deliver quote characters literally (if your original line
was quoted, they always got the quotes), and python-dotenv expands `${NAME}` inside values in
every quoting style unless it is called with `interpolate=False`.
...
- **`target_styles.json`, `placeholder_shapes.json` and `swap.journal.json` are plaintext and
  ...
  the agent could not have hidden by editing `.env` directly.
...
- Use `swap` only when the consumer is known to read the `.env` file and nothing else reaches
  ...
  show it to the human verbatim and wait.
